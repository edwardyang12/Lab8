describe('Basic user flow for SPA ', () => {
  ...
  it('Test9: On Settings page - checking <body> element classes', async () => {
    ...
    const classSetting = await page.$eval('body', (body) => {
      return body.className;
    });
    expect(classSetting).toBe("settings"); 
  });
  ...
  it('Test12: When the user if on the homepage, the header title should be “Journal Entries', async() => {
    const title = await page.$eval('header > h1', (heading) => {
  ...
  it('Test13: On the home page the <body> element should not have any class attribute', async() => {
    const list = await page.$eval('body', (header) => {
      return header.classList.length;
    });
    expect(list).toBe(0);  
  });
  ...
  it('Test17: Verify the url is correct when clicking on the last entry', async() => {
    await page.goBack();
    const entry = await page.$$('journal-entry');
    await entry[9].click();
  ...
  it('Test18: Verify the title is current when clicking on the last entry', async () => {
    const header = await page.$eval('header > h1', (entry) => {
      return entry.innerHTML;
    });
  ...
  it('Test19: Verify the entry page contents is correct when clicking on the last entry', async () => {

    const entrypage = await page.$('entry-page');
    const entry = await (await entrypage.getProperty('entry')).jsonValue();
    
    expect(entry.title).toBe("No, I am your father");
    expect(entry.date).toBe("5/4/2021");
    expect(entry.content).toBe("A long time ago, in a galaxy far, far away... It is a period of civil war. Rebel spaceships, striking from a hidden base, have won their first victory against the evil Galactic Empire. During the battle, Rebel spies managed to steal secret plans to the Empire's ultimate weapon, the Death Star, an armored space station with enough power to destroy an entire planet. Pursued by the Empire's sinister agents, Princess Leia races home aboard her starship, custodian of the stolen plans that can save her people and restore freedom to the galaxy....");
    expect(entry.image.src).toBe("https://starwarsblog.starwars.com/wp-content/uploads/2021/04/star-wars-may-the-4th-2021-TALL-3973202.jpg");
    expect(entry.image.alt).toBe("may the fourth be with you");
    expect(entry.audio).toBe("https://drive.google.com/uc?export=download&id=1luYh909US7ZBFe6uo440Vv_LNnRdnErT")
  }, 10000);


  // create your own test 20
  it('Test20: go back go forward idk how to sound smart', async () => {
    await page.goBack();
